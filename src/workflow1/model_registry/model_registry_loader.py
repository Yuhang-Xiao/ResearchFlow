...
from workflow1.paths import model_registry_root
...
def load_model_registry(root: str | Path | None = None) -> ModelRegistry:
    """Load all model and method registry files without executing external code."""

    registry_root = Path(root) if root is not None else model_registry_root()
