...
def main() -> None:
    ...
    write(
        "framework/workflow_improvement/README.md",
        """
        # Workflow Self-Improvement System

        `framework/workflow_improvement/` defines the long-term mechanism for letting Codex safely improve workflow1 itself.
    ...
    write(
        "framework/workflow_improvement/source_watchlist.yaml",
        """
        sources:
    ...
    write(
        "framework/workflow_improvement/capability_taxonomy.yaml",
        """
        capabilities:
    ...
    write(
        "framework/workflow_improvement/workflow_gap_schema.yaml",
        """
        fields:
          - capability
          - current_assets
    ...
    write(
        "framework/workflow_improvement/upgrade_evaluation_rubric.yaml",
        """
        score_fields:
    ...
    write(
        "framework/workflow_improvement/safe_patch_policy.yaml",
        """
        auto_allowed:
          - add local SKILL.md
          - add workflow recipe
    ...
    write(
        "framework/workflow_improvement/upgrade_backlog.yaml",
        """
        backlog:
    ...
    write(
        "framework/workflow_improvement/external_plugin_approval_queue.yaml",
        """
        queue:
    ...
    write(
        "framework/workflow_improvement/installed_or_adapted_components.yaml",
        """
        adapted_components:
          - id: workflow_self_improvement_system
            ...
            files:
              ...
              - .agents/skills/workflow-self-improvement-scout/SKILL.md
              - framework/workflow_recipes/workflow_self_improvement.yaml
            approval_needed: false
        """,
    )
    write_csv(
        "framework/workflow_improvement/improvement_ledger.csv",
        [
            {
    ...
    write_csv(
        "framework/workflow_improvement/rejected_candidates.csv",
        [
            {
    ...
    write(
        "framework/workflow_improvement/periodic_self_review_protocol.md",
        """
    ...
    skill_names = [
        ...
        ("github-skill-scout-and-adapter", "Search GitHub skill libraries, agents, and frameworks, then adapt safe ideas as local workflow1 assets."),
        ...
        ("skill-registry-doctor", "Check skill discoverability, trigger phrases, duplicate names, missing fields, and local skill registry health."),
    ]
...
"""
    ...
    sources_to_inspect:
      ...
      - project_state/
      - references/
    evaluation_rubric: framework/workflow_improvement/upgrade_evaluation_rubric.yaml
    allowed_actions:
      - add local skills
      - add recipes
    ...
    for name, triggers in recipes.items():
        write(
            f"framework/workflow_recipes/{name}.yaml",
            f"""
            name: {name}
            trigger_phrases:
            {chr(10).join('  - ' + t for t in triggers)}
    ...
    write(
        "framework/workflow_recipes/command_intents.yaml",
        """
        intents:
    ...
    write("framework/model_registry/README.md", "# Model Registry\n\nPlaceholder registry for method families, constraints, and approval status.\n")
    write("framework/model_registry/workflow_self_improvement_registry.yaml", "entries:\n  - id: workflow_self_improvement\n    status: active\n")

    # START_HERE and prompts.
    write(
        "framework/prompts/one_line_launchers.md",
        """
        # One-line Launchers
    ...
    write(
        ...
        - 新增 `framework/workflow_improvement/` 策略与 ledger。
        - 新增 8 个自我升级相关 skills，并写入 `.agents/skills/`。
        - 新增 5 个 workflow self-improvement recipes 和 command intents。
        - 新增 `src/workflow1/self_improvement/` 轻量 stub。
        - 新增 `framework/model_registry/` 占位注册表。
        - 更新 `START_HERE.md` 与 `framework/prompts/one_line_launchers.md`。
